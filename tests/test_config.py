...
class ConfigTests(unittest.TestCase):
    def test_default_system_prompt_allows_attached_media_answers(self) -> None:
        ...
        self.assertIn("Specific file read/explain/summarize/analyze requests require file content evidence", DEFAULT_SYSTEM_PROMPT)
        self.assertIn("return a content-reading command decision, not a directory listing", DEFAULT_SYSTEM_PROMPT)
        self.assertIn("If the target is a file path or filename, use a content-reading command", DEFAULT_SYSTEM_PROMPT)
        self.assertIn("Use directory listing only when the user asks to list files", DEFAULT_SYSTEM_PROMPT)
        self.assertIn('never use {"path":"..."} to answer about a file', DEFAULT_SYSTEM_PROMPT)
        ...
        self.assertIn("summarize README.md", DEFAULT_SYSTEM_PROMPT)
    ...
    def test_command_system_prompt_sends_local_hardware_queries_to_shell(self) -> None:
        ...
        self.assertIn("Specific file read/explain/summarize/analyze requests require file content evidence", ROUTE_SYSTEM_PROMPT)
        self.assertIn("return a content-reading command decision, not a directory listing", ROUTE_SYSTEM_PROMPT)
        self.assertIn("If the target is a file path or filename, use a content-reading command", ROUTE_SYSTEM_PROMPT)
        self.assertIn("Use directory listing only when the user asks to list files", ROUTE_SYSTEM_PROMPT)
        self.assertIn('never use {"path":"..."} to answer about a file', ROUTE_SYSTEM_PROMPT)
        ...
        self.assertIn("summarize README.md", ROUTE_SYSTEM_PROMPT)
