...
class NativeBuildCliTests(unittest.TestCase):
    ...
    def test_main_default_success_shows_phases_for_noop_build(self) -> None:
        stream = io.StringIO()
        with (
            tempfile.TemporaryDirectory() as tmp,
            contextlib.redirect_stdout(stream),
        ...
        self.assertEqual(code, 0)
        text = stream.getvalue()
        self.assertIn("==> checking toolchain", text)
        self.assertIn("==> preparing source/build dirs", text)
        self.assertIn("==> configuring CMake", text)
        self.assertIn("==> building native libraries", text)
        self.assertIn("==> copying/verifying libraries", text)
        self.assertIn("verified runtime libraries: libllama.so", text)
        self.assertIn("completed in", text)
        self.assertIn("next: orbit server --port 12120", text)
