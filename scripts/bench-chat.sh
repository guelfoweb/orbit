...
run_prompt() {
  ...
  prompt="$2"
  printf '\n## %s\n' "$name"
  /usr/bin/time -f 'wall: %e s' "$ORBIT" --max-tokens "$MAX_TOKENS" "$prompt"
}
...
printf 'Remember this keyword: lighthouse-cache.\nWhat keyword did I ask you to remember? Answer only the keyword.\n/exit\n' \
  | /usr/bin/time -f 'wall: %e s' "$ORBIT" --max-tokens 32
...
PY
  ...
  /usr/bin/time -f 'wall: %e s' "$ORBIT" --max-tokens 32 "$(cat "$tmp1")"
  printf '\n## long prefix cache second pass\n'
  /usr/bin/time -f 'wall: %e s' "$ORBIT" --max-tokens 32 "$(cat "$tmp2")"
fi
