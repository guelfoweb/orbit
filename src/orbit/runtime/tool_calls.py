...
def execute_tool_call(
    ...
    *,
    ...
    if not isinstance(function, dict):
        return ToolExecution(ToolResult(name="unknown", content="error: invalid tool call"), "orbit")
    name = function.get("name")
    arguments = function.get("arguments", {})
    tool_name = name if isinstance(name, str) else "unknown"
    return executor.execute(tool_name, arguments, chunk_budget=chunk_budget)
