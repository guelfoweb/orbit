...
using llama_decode_profile_clock = std::chrono::steady_clock;
using llama_decode_profile_time_point = llama_decode_profile_clock::time_point;

static bool llama_decode_profile_enabled() {
    static const bool enabled = getenv("GGML_SCHED_PROFILE_LOG") != nullptr;
    return enabled;
}

static long long llama_decode_profile_now_us() {
    return std::chrono::duration_cast<std::chrono::microseconds>(
        llama_decode_profile_clock::now().time_since_epoch()).count();
}

static double llama_decode_profile_elapsed_ms(llama_decode_profile_time_point start) {
    return std::chrono::duration<double, std::milli>(llama_decode_profile_clock::now() - start).count();
}

static FILE * llama_decode_profile_file() {
...
struct llama_decode_profile_accum {
    double sched_reserve_ms = 0.0;
    double memory_update_ms = 0.0;
    double output_reserve_ms = 0.0;
    double output_realloc_ms = 0.0;
    ...
    double output_extract_ms = 0.0;
    int ubatch_count = 0;
    int graph_reuse_count = 0;
    int graph_build_count = 0;
    int output_realloc_count = 0;
    size_t output_prev_size = 0;
    size_t output_new_size = 0;
};

thread_local llama_decode_profile_accum g_llama_decode_profile_accum;

static void llama_decode_profile_reset() {
    g_llama_decode_profile_accum = {};
}

static void llama_decode_profile_emit(
    ...
    if (!llama_decode_profile_enabled()) {
        return;
    }
    FILE * fp = llama_decode_profile_file();
    if (fp == nullptr) {
        return;
    }
    const auto & p = g_llama_decode_profile_accum;
    const double accounted_ms =
        p.sched_reserve_ms +
        p.memory_update_ms +
        p.output_reserve_ms +
        p.memory_init_batch_ms +
        p.graph_build_ms +
        p.graph_alloc_ms +
        p.set_inputs_ms +
        p.graph_compute_ms +
        p.output_extract_ms;
    const double other_ms = total_ms - accounted_ms;
    std::ostringstream out;
    out << "{"
        << "\"tag\":\"llama_decode_detail\""
        << ",\"started_us\":" << started_us
        << ",\"ctx\":\"" << (const void *) ctx << "\""
        << ",\"batch_n_tokens\":" << batch_n_tokens
        << ",\"batch_n_outputs\":" << batch_n_outputs
        << ",\"n_tokens_all\":" << n_tokens_all
        << ",\"n_outputs_all\":" << n_outputs_all
        << ",\"n_batch\":" << n_batch
        << ",\"n_ubatch\":" << n_ubatch
        << ",\"n_threads\":" << n_threads
        << ",\"n_threads_batch\":" << n_threads_batch
        << ",\"sched_reserve_ms\":" << p.sched_reserve_ms
        << ",\"memory_update_ms\":" << p.memory_update_ms
        << ",\"output_reserve_ms\":" << p.output_reserve_ms
        << ",\"output_realloc_ms\":" << p.output_realloc_ms
        << ",\"memory_init_batch_ms\":" << p.memory_init_batch_ms
        << ",\"graph_build_ms\":" << p.graph_build_ms
        << ",\"graph_alloc_ms\":" << p.graph_alloc_ms
        << ",\"set_inputs_ms\":" << p.set_inputs_ms
        << ",\"graph_compute_ms\":" << p.graph_compute_ms
        << ",\"output_extract_ms\":" << p.output_extract_ms
        << ",\"other_ms\":" << other_ms
        << ",\"ubatch_count\":" << p.ubatch_count
        << ",\"graph_reuse_count\":" << p.graph_reuse_count
        << ",\"graph_build_count\":" << p.graph_build_count
        << ",\"output_realloc_count\":" << p.output_realloc_count
        << ",\"output_prev_size\":" << p.output_prev_size
        << ",\"output_new_size\":" << p.output_new_size
        << ",\"total_ms\":" << total_ms
        << "}\n";
...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    if (!graph_reuse_disable && res->can_reuse(gparams)) {
        ...
        n_reused++;
        if (llama_decode_profile_enabled()) {
            g_llama_decode_profile_accum.graph_reuse_count++;
        }
    } else {
        res->reset();
        ...
        llama_decode_profile_time_point build_start;
        if (llama_decode_profile_enabled()) {
            build_start = llama_decode_profile_clock::now();
        }
        gf = model.build_graph(gparams);
        if (llama_decode_profile_enabled()) {
            g_llama_decode_profile_accum.graph_build_ms += llama_decode_profile_elapsed_ms(build_start);
            g_llama_decode_profile_accum.graph_build_count++;
        }
        ...
        llama_decode_profile_time_point alloc_start;
        if (llama_decode_profile_enabled()) {
            alloc_start = llama_decode_profile_clock::now();
        }
        if (!ggml_backend_sched_alloc_graph(sched.get(), gf)) {
            if (llama_decode_profile_enabled()) {
                g_llama_decode_profile_accum.graph_alloc_ms += llama_decode_profile_elapsed_ms(alloc_start);
            }
    ...
    {
        llama_decode_profile_time_point set_inputs_start;
        if (llama_decode_profile_enabled()) {
            set_inputs_start = llama_decode_profile_clock::now();
        }
        // FIXME this call causes a crash if any model inputs were not used in the graph and were therefore not allocated
        res->set_inputs(&ubatch);
        if (llama_decode_profile_enabled()) {
            g_llama_decode_profile_accum.set_inputs_ms += llama_decode_profile_elapsed_ms(set_inputs_start);
        }
    }

    llama_decode_profile_time_point compute_start;
    if (llama_decode_profile_enabled()) {
        compute_start = llama_decode_profile_clock::now();
        g_llama_decode_profile_accum.ubatch_count++;
    }
    const auto status = graph_compute(res->get_gf(), ubatch.n_tokens > 1);
    if (llama_decode_profile_enabled()) {
        g_llama_decode_profile_accum.graph_compute_ms += llama_decode_profile_elapsed_ms(compute_start);
    }
...
int llama_context::decode(const llama_batch & batch_inp) {
    const long long decode_started_us = llama_decode_profile_now_us();
    const auto decode_start = llama_decode_profile_clock::now();
    if (llama_decode_profile_enabled()) {
        llama_decode_profile_reset();
    }
    // MTP hook batches carry both token (next-token id) and embd (h_nextn row),
    // so accept either present rather than requiring exactly one.
    GGML_ASSERT(batch_inp.token || batch_inp.embd);
    ...
    {
        llama_decode_profile_time_point phase_start;
        if (llama_decode_profile_enabled()) {
            phase_start = llama_decode_profile_clock::now();
        }
        sched_reserve();
        if (llama_decode_profile_enabled()) {
            g_llama_decode_profile_accum.sched_reserve_ms += llama_decode_profile_elapsed_ms(phase_start);
        }
    }
    ...
    // handle any pending shifts/copies
    {
        ...
        memory_update(false);
        if (llama_decode_profile_enabled()) {
            g_llama_decode_profile_accum.memory_update_ms += llama_decode_profile_elapsed_ms(phase_start);
        }
    }
    ...
    while (true) {
        llama_decode_profile_time_point init_batch_start;
        if (llama_decode_profile_enabled()) {
            init_batch_start = llama_decode_profile_clock::now();
        }
        mctx = memory->init_batch(*balloc, cparams.n_ubatch, output_all);
        if (llama_decode_profile_enabled()) {
            g_llama_decode_profile_accum.memory_init_batch_ms += llama_decode_profile_elapsed_ms(init_batch_start);
        }
        if (!mctx) {
            return -2;
        }
    ...
    // reserve output buffer
    {
        ...
        if (output_reserve(n_outputs_all) < n_outputs_all) {
            if (llama_decode_profile_enabled()) {
                g_llama_decode_profile_accum.output_reserve_ms += llama_decode_profile_elapsed_ms(phase_start);
                llama_decode_profile_emit(this, decode_started_us, llama_decode_profile_elapsed_ms(decode_start), n_tokens_all, n_outputs_all, batch_inp.n_tokens, batch_inp.n_tokens, cparams.n_batch, cparams.n_ubatch, cparams.n_threads, cparams.n_threads_batch);
            }
    ...
    do {
        ...
        llama_decode_profile_time_point extract_start;
        if (llama_decode_profile_enabled()) {
            extract_start = llama_decode_profile_clock::now();
        }
        auto * t_logits  = res->get_logits();
        ...
        if (llama_decode_profile_enabled()) {
            g_llama_decode_profile_accum.output_extract_ms += llama_decode_profile_elapsed_ms(extract_start);
        }
    } while (mctx->next());
...
uint32_t llama_context::output_reserve(int32_t n_outputs) {
    ...
    if (!buf_output || prev_size < new_size) {
        const auto reserve_start = llama_decode_profile_enabled() ? llama_decode_profile_clock::now() : llama_decode_profile_time_point{};
        if (buf_output) {
#ifndef NDEBUG
        ...
        if (buf_output == nullptr) {
            if (llama_decode_profile_enabled()) {
                g_llama_decode_profile_accum.output_realloc_count++;
                g_llama_decode_profile_accum.output_prev_size = prev_size;
                g_llama_decode_profile_accum.output_new_size = new_size;
                g_llama_decode_profile_accum.output_realloc_ms += llama_decode_profile_elapsed_ms(reserve_start);
            }
