...
static bool mtp_contract_mismatch_trace_enabled() {
    const char * value = std::getenv("LLAMA_MTP_DRAFT_MISMATCH_TRACE");
    return value && value[0] && std::strcmp(value, "0") != 0;
}

static const char * mtp_trace_label() {
    const char * value = std::getenv("LLAMA_MTP_DRAFT_TRACE_LABEL");
    return (value && value[0]) ? value : "unknown";
}

static uint64_t mtp_hash_bytes(const uint8_t * data, size_t size) {
...
static uint64_t mtp_hash_string(const std::string & value) {
    return mtp_hash_bytes(reinterpret_cast<const uint8_t *>(value.data()), value.size());
}

static uint64_t mtp_hash_seq_state(llama_context * ctx, llama_seq_id seq_id, llama_state_seq_flags flags) {
    ...
    return mtp_hash_bytes(data.data(), n);
}

static std::string mtp_top_candidates_json(const llama_vocab * vocab, const llama_token_data_array * cur_p, int n_top) {
    std::ostringstream out;
    out << "[";
    const int limit = std::min<int>(n_top, (int) cur_p->size);
    for (int i = 0; i < limit; ++i) {
        if (i > 0) {
            out << ",";
        }
        const auto & tok = cur_p->data[i];
        out
            << "{"
            << "\"id\":" << (int) tok.id
            << ",\"piece\":\"" << common_token_to_piece(vocab, tok.id) << "\""
            << ",\"logit\":" << tok.logit
            << ",\"p\":" << tok.p
            << "}";
    }
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        common_batch_clear(batch);
        std::vector<llama_token> last_batch_tokens;
        std::vector<llama_pos> last_batch_pos;
        std::vector<llama_seq_id> last_batch_seq;
        int last_batch_n_outputs = 0;
        ...
        while (n_drafting > 0) {
            ...
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                ...
                auto * smpl = smpls[seq_id].get();
                const llama_token sampled = common_sampler_sample(smpl, ctx_dft, i_batch, true);
                h_row = llama_get_embeddings_nextn_ith(ctx_dft, i_batch);

                const auto * cur_p = common_sampler_get_candidates(smpl, true);
                const int selected_index = cur_p->selected;
                const llama_token selected_id =
                    (selected_index >= 0 && (size_t) selected_index < cur_p->size)
                        ? cur_p->data[selected_index].id
                        : LLAMA_TOKEN_NULL;
                const float selected_p =
                    (selected_index >= 0 && (size_t) selected_index < cur_p->size)
                        ? cur_p->data[selected_index].p
                        : 0.0f;
                ++i_batch;
                ...
                // Draft-MTP intentionally uses a greedy/top-1 frontier:
                // - common_sampler_sample() is still called so the sampler updates cur_p.selected
                //   and any sampler-side state remains observable for debugging/accounting
                // - the drafted token itself is the top-1 candidate after sorting by probability
                // - the same token must be used for sampler accept, spec_draft push, and the
                //   next decode input in order to keep the draft frontier self-consistent
                //
                // This is intentionally not a sampled-token draft policy. Replacing the draft
                // token with `sampled` changes the autoregressive frontier and regresses the
                // MTP path materially on real prompts.
                const llama_token id = cur_p->data[0].id;
                const float top1_p = cur_p->size > 0 ? cur_p->data[0].p : 0.0f;
                const bool gate_pass = top1_p >= params.p_min;

                if (mtp_contract_mismatch_trace_enabled() && sampled != id) {
                    const llama_vocab * vocab = llama_model_get_vocab(llama_get_model(ctx_dft));
                    std::ostringstream out;
                    out
                        << "{"
                        << "\"label\":\"" << mtp_trace_label() << "\""
                        << ",\"seq_id\":" << (int) seq_id
                        << ",\"draft_index\":" << i
                        << ",\"id_last\":" << (int) dparams[seq_id].id_last
                        << ",\"n_past\":" << dparams[seq_id].n_past
                        << ",\"sampled\":{\"id\":" << (int) sampled
                        << ",\"piece\":\"" << common_token_to_piece(vocab, sampled) << "\"}"
                        << ",\"selected_index\":" << selected_index
                        << ",\"selected\":{\"id\":" << (int) selected_id
                        << ",\"piece\":\"" << common_token_to_piece(vocab, selected_id) << "\""
                        << ",\"p\":" << selected_p << "}"
                        << ",\"top1\":{\"id\":" << (int) id
                        << ",\"piece\":\"" << common_token_to_piece(vocab, id) << "\""
                        << ",\"p\":" << top1_p << "}"
                        ...
                        << ",\"piece\":\"" << common_token_to_piece(vocab, id) << "\"}"
                        << "}";
                    std::fprintf(stderr, "LLAMA_MTP_DRAFT_MISMATCH %s\n", out.str().c_str());
                }
                ...
                GGML_ASSERT(batch.token[batch.n_tokens - 1] == id);
                std::memcpy(batch.embd + n_embd*(batch.n_tokens - 1), h_row, row_bytes);
            }
            ...
            last_batch_tokens.clear();
            last_batch_pos.clear();
            last_batch_seq.clear();
            last_batch_n_outputs = 0;
            for (int bi = 0; bi < batch.n_tokens; ++bi) {
                last_batch_tokens.push_back(batch.token[bi]);
                last_batch_pos.push_back(batch.pos[bi]);
                last_batch_seq.push_back(batch.n_seq_id[bi] > 0 ? batch.seq_id[bi][0] : -1);
                if (batch.logits && batch.logits[bi]) {
                    last_batch_n_outputs++;
                }
            }
