...
- **GGUF-MULTISHARD-DOWNLOAD-21 (2026-09-16, #357) — native split GGUF
  ...
  - **Split detection (`native_llama/gguf_split.py`):** a file name matching
    `<base>-NNNNN-of-MMMMM.gguf` (five-digit zero-padded index and count, as
  ...
  - **Download / resume (`model_download.py`):** `download_model` routes split
    ...
    resuming)` per shard. `orbit download --all <repo>` passes through the
    same path. The single-file path (urlretrieve into `.<name>.tmp` +
    `os.replace`) is unchanged.
  - **Completion validation (`validate_split_set` / `split_problems`):** every
    shard exists and is non-empty, has GGUF magic and version 2/3, `split.count`
    equals the count in the name, `split.no` equals its 0-based position, and
  ...
  - **Tests:** `tests/test_gguf_multishard.py` (A–J + failure matrix, all
    offline with synthetic GGUF shards and a Range-aware fake opener that can
    drop a transfer mid-shard, ignore Range, or close early): single GGUF
    unchanged, 3-shard and N-shard expansion, complete set AVAILABLE, missing
    shard INCOMPLETE/never AVAILABLE, corrupt/inconsistent/malformed rejected,
    shard-2 network failure keeps shard 1 + partial 2 and resumes only what is
